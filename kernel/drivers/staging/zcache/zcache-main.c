...
#define WMSIZE		ZCACHE_DSTMEM_ORDER
...
#else
#error either CONFIG_CRYPTO_LZO or CONFIG_ZCACHE_SNAPPY must be defined
#endif
...
#if defined(CONFIG_ZCACHE_LZO)
	ret = zcache_comp_op(ZCACHE_COMPOP_DECOMPRESS, from_va, size, to_va, &out_len);
	BUG_ON(ret);
#elif defined(CONFIG_ZCACHE_SNAPPY)
...
static void zv_decompress(struct page *page, struct zv_hdr *zv)
{
	...
	to_va = kmap_atomic(page, KM_USER0);
#if defined(CONFIG_ZCACHE_LZO)
	ret = zcache_comp_op(ZCACHE_COMPOP_DECOMPRESS, (char *)zv + sizeof(*zv),size, to_va, &clen);
	kunmap_atomic(to_va, KM_USER0);
	BUG_ON(ret);
#elif defined(CONFIG_ZCACHE_SNAPPY)
	ret = DECOMPRESS((char *)zv + sizeof(*zv), size, to_va, &clen);
	kunmap_atomic(to_va, KM_USER0);
	BUG_ON(ret != CSNAPPY_E_OK);
#endif
...
static int zcache_compress(struct page *from, void **out_va, unsigned *out_len)
{
	...
	*out_len = PAGE_SIZE << ZCACHE_DSTMEM_ORDER;
	...
	mb();
#if defined(CONFIG_ZCACHE_LZO)
	ret = zcache_comp_op(ZCACHE_COMPOP_COMPRESS, from_va, PAGE_SIZE, dmem, out_len);
	BUG_ON(ret);
#elif defined(CONFIG_ZCACHE_SNAPPY)
	ret = COMPRESS(from_va, PAGE_SIZE, dmem, out_len, ZCACHE_COMPOP_COMPRESS);
	BUG_ON(ret != CSNAPPY_E_OK);
#endif
	*out_va = dmem;
				...
				unsigned long action, void *pcpu)
{
	...
	case CPU_UP_PREPARE:
		...
		per_cpu(zcache_dstmem, cpu) = (void *)__get_free_pages(
			GFP_KERNEL | __GFP_REPEAT, WMSIZE_ORDER);
		break;
	case CPU_DEAD:
